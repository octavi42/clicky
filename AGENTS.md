...
| `SimulationControlPanelView.swift` | ~840 | Presenter-only "Clicky Memory Demo" cockpit with a minimalist single-card-recipe design (demo state strip, equal-height feature demo card rows, capsule chips). Live: demo state strip, profile chips, reset, the Skills, Preferences, and Routines feature cards (compact dashboards with proof fields whose Run opens the before/after comparison window), and the proof panel. The Niche card and Ask Clicky quick actions are placeholders. Also home of the shared `SimulationControlPanelChipButton` and `simulationCardSurface()` used by the comparison window. |
...
| `SimulationDemoEngine.swift` | ~1250 | Deterministic state engine for the memory demo. Reset-to-baseline, demo profile loading, and a generic beat-based demo script player (`DemoConversationBeat` / `DemoScriptStep`, each step tagged with a `DemoComparisonLane`): scripts play as paced before/after conversations in the comparison window while real side effects fire at exact beats; only the conversation text is scripted. Three scripts so far, identified by `FeatureDemoKind`: Skills (real `TeachingSkillStore` write + `SkillMatcher` + `TeachingPromptBuilder`), Preferences (real `PreferenceSignalDetector` + `AuxiliaryMemoryStore` write + `TeachingPromptBuilder` preference injection), and Routines (real backdated `ActivityStore` transition writes + `RoutineDetector` recurrence rules producing the routine chip, pushed into the live companion panel). One shared run task — starting a demo cancels the others. Publishes per-demo run states/proof fields, per-lane beats, the typing lane, recap text, demo-state counts, and proof panel fields. |
