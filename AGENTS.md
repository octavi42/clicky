...
| `SimulationControlPanelWindowManager.swift` | ~91 | Manages the dedicated titled, resizable NSWindow hosting the presenter-only "Clicky Memory Demo" control panel. Single reusable window instance; opens via the `.clickyOpenSimulationControlPanel` notification and activates the app explicitly (LSUIElement apps don't activate on their own). |
| `SimulationControlPanelView.swift` | ~434 | Presenter-only "Clicky Memory Demo" cockpit skeleton. Placeholder sections for demo state, the four feature demo cards (skills, preferences, routines, niche suggestions), demo profiles, Ask Clicky quick actions, and the proof panel. Demo actions are not wired yet. |
