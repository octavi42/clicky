...
| `SimulationControlPanelView.swift` | ~700 | Presenter-only "Clicky Memory Demo" cockpit with a minimalist single-card-recipe design (demo state strip, equal-height feature demo card rows, capsule chips). Live: demo state strip, profile chips, reset, the Skills feature card (compact dashboard with proof fields whose Run opens the before/after comparison window), and the proof panel. The remaining feature cards and Ask Clicky quick actions are placeholders. Also home of the shared `SimulationControlPanelChipButton` and `simulationCardSurface()` used by the comparison window. |
| `SimulationDemoComparisonView.swift` | ~496 | Before/after comparison screen for feature demo runs: two side-by-side conversation columns ("First time" plays the multi-turn teaching session beat by beat; "Next day" waits dimmed, then plays the one-turn matched re-ask), per-column auto-scroll, typing indicator, system event pills, live asks counters, recap strip spanning both columns, Replay chip. Demo-agnostic — renders whatever lane beats the engine publishes. |
| `SimulationDemoComparisonWindowManager.swift` | ~93 | Manages the dedicated titled, resizable NSWindow hosting the demo comparison screen. Single reusable window instance; opens via the `.clickyOpenSimulationDemoComparisonWindow` notification (posted by the Skills card's Run button) and activates the app explicitly. Separate from the cockpit window so the presenter can drag it to a projector. |
| `SimulationDemoEngine.swift` | ~593 | Deterministic state engine for the memory demo. Reset-to-baseline, demo profile loading, and a generic beat-based demo script player (`DemoConversationBeat` / `DemoScriptStep`, each step tagged with a `DemoComparisonLane`): scripts play as paced before/after conversations in the comparison window while real side effects fire at exact beats (real `TeachingSkillStore` write + real `SkillMatcher` + real `TeachingPromptBuilder`; only the conversation text is scripted). The Skills demo is the first script. Publishes per-lane beats, the typing lane, recap text, demo-state counts, and proof panel fields. |
