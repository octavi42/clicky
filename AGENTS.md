...
| `CompanionPanelView.swift` | ~850 | SwiftUI panel content for the menu bar dropdown. Shows companion status, push-to-talk instructions, niche onboarding/suggestions, teaching skills UI, model picker (Sonnet/Opus), permissions UI, DM feedback button, and quit button. DEBUG builds add an "Open Memory Demo" button in the settings tab that opens the simulation control panel window. Dark aesthetic using `DS` design system. |
