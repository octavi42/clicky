...
| `SimulationControlPanelView.swift` | ~560 | Presenter-only "Clicky Memory Demo" cockpit with a minimalist single-card-recipe design (demo state strip, equal-height feature demo card rows, capsule chips). Live demo state strip, demo profile chips, and reset are wired to `SimulationDemoEngine`; the four feature demo cards, Ask Clicky quick actions, and proof panel are still placeholders. |
| `SimulationDemoEngine.swift` | ~173 | Deterministic state engine for the memory demo. Reset-to-baseline (wipe skills, auxiliary memories, activity edges, niche override) and demo profile loading over CompanionManager's shared store instances, plus published demo-state counts for the cockpit tiles. |
