...
| `SimulationControlPanelView.swift` | ~1060 | Presenter-only "Clicky Memory Demo" cockpit with a minimalist single-card-recipe design (demo state strip, equal-height feature demo card rows, capsule chips). Live: demo state strip, profile chips, reset, the Skills feature card (compact dashboard with proof fields), the demo stage (conversation replay with user/Clicky bubbles, typing indicator, system event pills, time-gap divider, recap strip, auto-scroll, Replay chip), and the proof panel. The remaining feature cards and Ask Clicky quick actions are placeholders. |
| `SimulationDemoEngine.swift` | ~551 | Deterministic state engine for the memory demo. Reset-to-baseline, demo profile loading, and a generic beat-based demo script player (`DemoConversationBeat` / `DemoScriptStep`): scripts play as paced conversations on the stage while real side effects fire at exact beats (real `TeachingSkillStore` write + real `SkillMatcher` + real `TeachingPromptBuilder`; only the conversation text is scripted). The Skills demo is the first script. Publishes demo-state counts, stage beats, typing flag, and proof panel fields. |
